...
# Match write blocks including reassignments for baos, rawData, serializedEvent
write_block_re = re.compile(
    ...
    r'[ \t]*(?:XMLObjectWriter\s+writer\s*=|writer\s*=)\s*XMLObjectWriter\.newInstance\(baos\)\s*;\s*\n'
    r'(?:[ \t]*//[^\n]*\n)*'
    r'[ \t]*writer\.setIndentation\("[^"]*"\)\s*;\s*(?://[^\n]*\n)*'
    r'[ \t]*writer\.write\((?P<var>[^,]+),\s*"(?P<tag>[^"]+)"\,\s*(?P<clazz>[^)]+)\)\s*;\s*\n'
    r'[ \t]*writer\.close\(\)\s*;\s*\n\s*\n'
    r'[ \t]*(?:byte\[\]\s+rawData\s*=|rawData\s*=)\s*baos\.toByteArray\(\)\s*;\s*\n'
    r'[ \t]*(?:String\s+serializedEvent\s*=|serializedEvent\s*=)\s*new\s+String\(rawData\)\s*;\s*\n\s*\n'
    r'[ \t]*System\.out\.println\(serializedEvent\)\s*;',
    re.MULTILINE
)

# Match read blocks including reassignments for bais
read_block_re = re.compile(
    ...
    r'[ \t]*(?:XMLObjectReader\s+reader\s*=|reader\s*=)\s*XMLObjectReader\.newInstance\(bais\)\s*;\s*\n'
    r'[ \t]*(?P<copyType>[A-Za-z0-9_<>]+)\s+(?P<copyVar>[a-zA-Z0-9_]+)\s*=\s*reader\.read\("(?P<tag>[^"]+)",\s*(?P<clazz>[^)]+)\)\s*;',
    re.MULTILINE
)
...
def process_file(filepath):
    ...
    if 'XMLObjectWriter' not in content and 'XMLObjectReader' not in content:
        return False
    ...
    def replace_read(m):
        ...
        copy_type = m.group('copyType').strip()
        copy_var = m.group('copyVar').strip()
        decl_clazz = clazz
        if decl_clazz.endswith('.class'):
            decl_clazz = decl_clazz[:-6]
        return (
            f'{indent}{decl_clazz} {copy_var} = xmlMapper.readValue(serializedEvent, {clazz});'
        )
