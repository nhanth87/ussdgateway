...
def fix_file(filepath):
    ...
    while i < len(lines):
        ...
        # Detect start of write block (baos or output)
        if (('ByteArrayOutputStream baos = new ByteArrayOutputStream()' in line) or
            ('baos = new ByteArrayOutputStream()' in line) or
            ('ByteArrayOutputStream output = new ByteArrayOutputStream()' in line) or
            ('output = new ByteArrayOutputStream()' in line)):
            block = [line]
            j = i + 1
            var_name = None
            clazz_name = None
            uses_output = 'output' in line
            while j < len(lines):
                ...
                if ('System.out.println(serializedEvent);' in lines[j] or
                    'System.out.println(output.toString());' in lines[j]):
                    break
                j += 1
        ...
        # Detect start of read block (bais or input)
        if (('ByteArrayInputStream bais = new ByteArrayInputStream(rawData)' in line) or
            ('bais = new ByteArrayInputStream(rawData)' in line) or
            ('ByteArrayInputStream input = new ByteArrayInputStream(output.toByteArray())' in line) or
            ('input = new ByteArrayInputStream(output.toByteArray())' in line)):
            block = [line]
            j = i + 1
            tag_name = None
            clazz_name = None
