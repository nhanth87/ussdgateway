...
public class TcapManagementJmx implements TcapManagementJmxMBean, CounterMediator {
    ...
    private void setupCounterList() {
        ...
        cd = new CounterDefImpl(CounterType.ComplexValue, "OutgoingDialogsPerApplicationContextName", "An outgoing Dialogs count per ApplicationContextNames (in string form)");
        cds.addCounterDef(cd);
        cd = new CounterDefImpl(CounterType.ComplexValue, "IncomingDialogsPerApplicationContextName", "An incoming Dialogs count per ApplicationContextNames (in string form)");
        cds.addCounterDef(cd);
    ...
    public SourceValueSet getSourceValueSet(String counterDefSetName, String campaignName, int durationInSeconds) {
        ...
        try {
            ...
            for (CounterDef cd : cds.getCounterDefs()) {
                ...
                } else if (cd.getCounterName().equals("OutgoingDialogsPerApplicationContextName")) {
                    svo = createComplexValue(cp.getOutgoingDialogsPerApplicationContextName(campaignName));
                } else if (cd.getCounterName().equals("IncomingDialogsPerApplicationContextName")) {
                    svo = createComplexValue(cp.getIncomingDialogsPerApplicationContextName(campaignName));
    ...
    /* (non-Javadoc)
     * @see org.restcomm.protocols.ss7.tcap.api.TCAPStack#isSwapTcapIdBytes()
     */
    @Override
    public boolean getSwapTcapIdBytes() {
        return this.wrappedTCAPStack.getSwapTcapIdBytes();
    }

    /* (non-Javadoc)
     * @see org.restcomm.protocols.ss7.tcap.api.TCAPStack#setSwapTcapIdBytes(boolean)
     */
    @Override
    public void setSwapTcapIdBytes(boolean arg0) {
        this.wrappedTCAPStack.setSwapTcapIdBytes(arg0);
    }
