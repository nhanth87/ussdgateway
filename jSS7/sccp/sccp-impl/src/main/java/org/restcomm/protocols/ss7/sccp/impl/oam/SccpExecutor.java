...
public class SccpExecutor implements ShellExecutor {
        ...
        if (command.equals("create")) {
            ...
            while (count < options.length) {
                ...
                if (key.equals("stackname")) {
                    ...
                    remoteSpc = Integer.parseInt(options[count++]);
                }  else if (key.equals("rspc-flag")) {
                    remoteSpcFlag = Integer.parseInt(options[count++]);
                }  else if (key.equals("mask")) {
                    mask = Integer.parseInt(options[count++]);
                } else {
                    return SccpOAMMessage.INVALID_COMMAND;
                }
    ...
    private String manageLmr(String[] options) throws Exception {
        ...
        if (command.equals("create")) {
            ...
            Map<Integer, LongMessageRule> idVsLongMsgRule = this.sccpStack.getRouter().getLongMessageRules();

            StringBuffer sb = new StringBuffer();
            for (Integer e : idVsLongMsgRule.keySet()) {
                LongMessageRule lmr = idVsLongMsgRule.get(e);
                sb.append("key=");
                sb.append(e);
                sb.append("  ");
                sb.append(lmr);
    ...
    private String manageSap(String[] options) throws Exception {
        ...
        if (command.equals("create")) {
            ...
            // -99 is used as a default value if the actual value is not passed. In such case, the existing value will be used instead of -99
            int mtp3Id = -99;
            int opc = -99;
            int ni = -99;
            int networkId = -99;
            ...
            while (count < options.length - 1) {
                ...
                if (key.equals("stackname")) {
                    ...
                    mtp3Id = Integer.parseInt(options[count++]);
                } else if (key.equals("opc")) {
                    opc = Integer.parseInt(options[count++]);
                } else if (key.equals("ni")) {
                    ni = Integer.parseInt(options[count++]);
                } else if (key.equals("networkid")) {
                    networkId = Integer.parseInt(options[count++]);
                } else if (key.equals("localgtdigits")) {
                    localGtDigits = options[count++];
                } else {
                    return SccpOAMMessage.INVALID_COMMAND;
            ...
            if (options.length > 11) {
                ...
                while (count < options.length) {
                    ...
                    if (key.equals("stackname")) {
                        ...
                        firstDpc = Integer.parseInt(options[count++]);
                    } else if (key.equals("last-dpc")) {
                        lastDpc = Integer.parseInt(options[count++]);
                    } else if (key.equals("first-sls")) {
                        firstSls = Integer.parseInt(options[count++]);
                    } else if (key.equals("last-sls")) {
                        lastSls = Integer.parseInt(options[count++]);
                    } else if (key.equals("sls-mask")) {
                        slsMask = Integer.parseInt(options[count++]);
                    } else {
                        return SccpOAMMessage.INVALID_COMMAND;
                    }
                }
            } else {
                firstDpc = Integer.parseInt(options[5]);
                lastDpc = Integer.parseInt(options[6]);
                firstSls = Integer.parseInt(options[7]);
                lastSls = Integer.parseInt(options[8]);
                slsMask = Integer.parseInt(options[9]);
                int count = 10;
    ...
    private String manageGet(String[] options) throws Exception {
        ...
        if (options.length == 3) {
            ...
            if (parName.equals("zmarginxudtmessage")) {
                ...
                sb.append(this.sccpStack.isCongControl_blockingOutgoingSccpMessages());
            } else {
                return SccpOAMMessage.INVALID_COMMAND;
            }
            ...
            for (Map.Entry<String, SccpStackImpl> e : this.sccpStacks.entrySet()) {
                ...
                sb.append(managementImplTmp.isCongControl_blockingOutgoingSccpMessages());
                sb.append("\n");
