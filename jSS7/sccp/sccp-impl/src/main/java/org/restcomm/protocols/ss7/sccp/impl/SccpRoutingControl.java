...
public class SccpRoutingControl implements SccpRoutingCtxInterface {
    ...
    protected void routeMsgFromMtp(SccpAddressedMessageImpl msg) throws Exception {
...
//                        listener.onMessage((SccpDataMessage) msg);
    ...
    protected void routeMsgFromMtpConn(SccpConnMessage msg) throws Exception {
    ...
    private long lastCongAnnounceTime;
    ...
    protected void send(SccpAddressedMessageImpl message) throws Exception {
        ...
        if (this.sccpStackImpl.isCongControl_blockingOutgoingSccpMessages() && msgImportance < currentRestrictionLevel) {
            // we are dropping a message because of outgoing congestion

            long curTime = System.currentTimeMillis();
            if (lastCongAnnounceTime + 1000 < curTime) {
                lastCongAnnounceTime = curTime;
...
//                return erd.getReturnCause();
    ...
    public void routeAddressed(SccpAddressedMessageImpl msg) throws Exception {
        ...
        if (calledPartyAddress.getAddressIndicator().isPCPresent()) {
            // DPC present
            logger.debug("Processing in routeAddressed and DPC present");
            if (this.sccpStackImpl.router.spcIsLocal(dpc, msg.getNetworkId())) {
...
//                            conn.receiveMessage(cr);
                ...
                logger.debug("DPC present but its not local pointCode. This message should be Tx to MTP");
            ...
            if (gt == null) {
                ...
                if (logger.isEnabledFor(Level.WARN)) {
                    logger.warn(String
                            .format("Received SccpMessage=%s for routing from local SCCP user part but no point code and no GT or SSN included",
                                    msg, dpc));
                }
                this.sendSccpError(msg, ReturnCauseValue.NO_TRANSLATION_FOR_NATURE, RefusalCauseValue.NO_TRANSLATION_FOR_AN_ADDRESS_OF_SUCH_NATURE);
                return;
