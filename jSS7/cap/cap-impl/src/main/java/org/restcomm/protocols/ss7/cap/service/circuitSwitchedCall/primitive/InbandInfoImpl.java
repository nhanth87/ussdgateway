...
public class InbandInfoImpl extends SequenceBase implements InbandInfo {
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_messageID);
            int pos = asnOutputStream.StartContentDefiniteLength();
            ((MessageIDImpl) this.messageID).encodeAll(asnOutputStream);
            asnOutputStream.FinalizeContent(pos);

            if (this.numberOfRepetitions != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _ID_numberOfRepetitions, this.numberOfRepetitions);
            if (this.duration != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _ID_duration, this.duration);
            if (this.interval != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _ID_interval, this.interval);
