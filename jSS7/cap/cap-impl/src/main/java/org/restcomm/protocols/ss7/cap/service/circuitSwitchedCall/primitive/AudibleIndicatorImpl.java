...
public class AudibleIndicatorImpl implements AudibleIndicator, CAPAsnPrimitive {
    ...
    private void _decode(AsnInputStream asnInputStream, int length) throws CAPParsingComponentException, MAPParsingComponentException,
        ...
        if (asnInputStream.getTagClass() == Tag.CLASS_UNIVERSAL) {
            ...
            case Tag.BOOLEAN:
                ...
                this.tone = asnInputStream.readBooleanData(length);
                break;
            ...
            case _ID_burstList:
                ...
                ((BurstListImpl) this.burstList).decodeData(asnInputStream, length);
                break;
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        try {
            if (tone != null) {
                asnOutputStream.writeBooleanData(tone);
                return;
            } else if (burstList != null) {
                ((BurstListImpl) burstList).encodeData(asnOutputStream);
                return;
            }
        } catch (IOException e) {
