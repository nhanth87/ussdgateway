...
public class CallGapRequestImpl extends CircuitSwitchedCallMessageImpl implements CallGapRequest {
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            ...
            asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_gapCriteria);
            int pos = asnOutputStream.StartContentDefiniteLength();
            ((GapCriteriaImpl) this.gapCriteria).encodeAll(asnOutputStream);
            asnOutputStream.FinalizeContent(pos);

            ((GapIndicatorsImpl) this.gapIndicators).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, _ID_gapIndicators);

            if (this.controlType != null) {
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _ID_controlType, this.controlType.getCode());
            }
            if (this.gapTreatment != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_gapTreatment);
            ...
            if (this.capExtensions != null) {
                ((CAPExtensionsImpl) this.capExtensions).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, _ID_capExtension);
            }
