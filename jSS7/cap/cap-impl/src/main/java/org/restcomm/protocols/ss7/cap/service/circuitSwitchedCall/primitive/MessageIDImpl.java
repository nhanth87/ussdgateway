...
public class MessageIDImpl implements MessageID, CAPAsnPrimitive {
    ...
    private void _decode(AsnInputStream asnInputStream, int length) throws CAPParsingComponentException, IOException, AsnException {
        ...
        switch (asnInputStream.getTag()) {
            case _ID_elementaryMessageID:
                this.elementaryMessageID = (int) asnInputStream.readIntegerData(length);
                break;
            case _ID_text:
                this.text = new MessageIDTextImpl();
                ((MessageIDTextImpl) this.text).decodeData(asnInputStream, length);
                break;
            ...
            case _ID_variableMessage:
                ...
                ((VariableMessageImpl) this.variableMessage).decodeData(asnInputStream, length);
                break;
            default:
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            if (this.elementaryMessageID != null)
                asnOutputStream.writeIntegerData(this.elementaryMessageID);
            if (this.text != null)
            ...
            if (this.elementaryMessageIDs != null) {
                ...
                for (Integer val : this.elementaryMessageIDs) {
                    ...
                    asnOutputStream.writeInteger(val);
                }
            }
            if (this.variableMessage != null)
