 ...
 *
 */
@JacksonXmlRootElement(localName = "variablePart")
public class VariablePartImpl implements VariablePart, CAPAsnPrimitive {
    ...
    private void _decode(AsnInputStream asnInputStream, int length) throws CAPParsingComponentException, IOException, AsnException {
        ...
        switch (asnInputStream.getTag()) {
            case _ID_integer:
                this.integer = (int) asnInputStream.readIntegerData(length);
                break;
            case _ID_number:
                this.number = new DigitsImpl();
                ((DigitsImpl) this.number).decodeData(asnInputStream, length);
                this.number.setIsGenericDigits();
                break;
            case _ID_time:
                this.time = new VariablePartTimeImpl();
                ((VariablePartTimeImpl) this.time).decodeData(asnInputStream, length);
                break;
            case _ID_date:
                this.date = new VariablePartDateImpl();
                ((VariablePartDateImpl) this.date).decodeData(asnInputStream, length);
                break;
            case _ID_price:
                this.price = new VariablePartPriceImpl();
                ((VariablePartPriceImpl) this.price).decodeData(asnInputStream, length);
                break;
            default:
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            if (this.integer != null)
                asnOutputStream.writeIntegerData(this.integer);
            if (this.number != null)
