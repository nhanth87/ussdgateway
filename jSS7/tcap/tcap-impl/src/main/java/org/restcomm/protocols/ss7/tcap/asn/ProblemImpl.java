...
public class ProblemImpl implements Problem {
    ...
    /**
     * @param problemType the type to set
     */
        ...
        try {
            switch (type) {
                case General:
                    ...
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, (int) type.getTypeTag(), this.generalProblemType.getType());
                    break;

                case Invoke:
                    if (this.invokeProblemType == null)
                        throw new EncodeException("Problem Type is Invoke, no specific type set");
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, (int) type.getTypeTag(), this.invokeProblemType.getType());
                    break;

                case ReturnError:
                    if (this.returnErrorProblemType == null)
                        throw new EncodeException("Problem Type is ReturnError, no specific type set");
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, (int) type.getTypeTag(), this.returnErrorProblemType.getType());
                    break;

                case ReturnResult:
                    if (this.returnResultProblemType == null)
                        throw new EncodeException("Problem Type is Result, no specific type set");
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, (int) type.getTypeTag(),
