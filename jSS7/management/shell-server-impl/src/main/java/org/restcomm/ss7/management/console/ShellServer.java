...
public abstract class ShellServer extends Task implements ShellServerMBean {
    ...
    public long perform() {
        ...
        try {
            ...
            logger.error("An error occurred while selecting selector key", ce);
            if (ce.getKey() != null) {
            ...
            logger.error("An error occurred while selecting selector key", e);
        }
                ...
                for (ChannelSelectionKey key : keys) {
                    ...
                    if (!key.isValid()) {
                        ...
                        try {
                            ...
                            if (msg != null) {
                                ...
                                if (rxMessage.compareTo("disconnect") == 0) {
                                    ...
                                    if (!isAuthManagementLoaded()) {
                                        logger.error("Cannot authenticate because AuthenticationManagement is null!");
                            ...
                            logger.error(
                                    "IO Exception while operating on ChannelSelectionKey. Client CLI connection shall close now",
                                    e);
                            try {
                            ...
                            logger.error(
                                    "Exception while operating on ChannelSelectionKey. Client CLI connection shall close now",
                                    e);
                            try {
                                this.closeChannel(key, chan);
                            } catch (IOException e1) {
        ...
        public long perform() {
            try {
                ...
                try {
                    ...
                    logger.error("An error occurred while waiting on command response", ex);
                }
                ...
                logger.error("IO Exception while operating on ChannelSelectionKey. Client CLI connection shall close now",
                        e);
                try {
                ...
                logger.error("Exception while operating on ChannelSelectionKey. Client CLI connection shall close now", e);
                try {
                    closeChannel(key, chan);
                } catch (IOException e1) {
                    logger.error("IO Exception while closing Channel", e1);
